...
> **Option 3** — full version with per-pattern diagrams at full size and
> a detailed paragraph under each. Likely runs ~7–8 pages when exported
> to PDF (over the 5-page rubric limit). Submit this only if the
> 5-page limit is not strictly enforced.
